...
sub GetSeqJustNucs {
    ...
    return static_GetJustNucsFromSeq($seq);
}
...
sub GetSeqNormalized { # gaps are always periods, U is always used instead of T, nucs are always upper case
    my ($sto,$hit)=@_;
    my $seq=$sto->GetSeq($hit);
    $seq=uc($seq);
    $seq =~ s/[^A-Z]/./g;
...
sub ProjectToColumnInterval {
    ...
    for my $srcHit ($src->ListHits()) {
	
	# calculate coord trims
	my $seq=$src->{hit}{$srcHit}{seq};
	my $lseq=length($seq);
	if ($l<0 || $l>length($seq)) {
	    die "oops.  l=$l, r=$r, lseq=$lseq, srcHit=$srcHit for $src->{stoFileName}";
	}
...
sub static_GetHitListFromStockholmString { # the point of this function is that I suspect actually Load'ing it might be time-consuming in some cases.  In one test, it was ~15 times faster.  DOES NOT INCLUDE duplicates in the result
    my ($string)=@_;
    my @hitList=();
    for my $line (split /\n/,$string) {
	if ($line eq "") {
	...
	my $firstCh=substr($line,0,1);
	if ($firstCh eq "/" || $firstCh eq "#") {
	    next;
	}
	if ($line =~ /^([^ \t]+)[ \t]+([^ \t]+)$/) {
	    my ($hit,$seq)=($1,$2);
...
sub Save {
    ...
    # $options->{allowNoHits} : allow saving a file that has no hits in it
    # $options->{ignoreSanityChecks} : don't 'die' when sanity checks fail, which is mainly intended so we can see what the file looks like and diagnose the problem
	...
	my $msg="(Stockholm::Save($destFile)) some hits in \%seqs were not given in \@{\$seqs{hitsInOriginalOrder}}: ".join(",",keys %hitsNotSeenForDebugging);
	if ($options->{ignoreSanityChecks}) {
	    print "WARNING: $msg\n";
	}
	else {
	    die $msg;
...
sub ExpandDuplicatesForRedundantMsa {
    my ($self,$allowDuplicateInMsa,$alreadyRemovedHitListRef,$ignoreUnfoundReferenceHit)=@_;
    ...
    if (1) {
	# remove hits with identical coords
	my @newHits=();
	my %seenHit=();
	for my $hit ($self->ListHits()) {
	    if ($seenHit{$hit}) {
...
sub GetHitWeightOrOne {
    my ($sto,$hit)=@_;
    my $weight=$sto->{hit}{$hit}{gs}{WT};
    if (!defined($weight)) {
	$weight=1.0;
    }
    return $weight;
...
sub GetColGapWeight {
    ...
    for my $hit ($sto->ListHits()) {
	my $seq=$sto->GetSeq($hit);
	my $s=substr($seq,$col,1);
	if ($s eq "X" || $s eq "x") { # ignore unknown protein amino acids
	    next;
	}
	my $weight=$sto->GetHitWeightOrOne($hit);
	if ($s =~ /[^A-Za-z]/) {
	    $pgap += $weight;
	}
	$ptotal += $weight;
    }
    $pgap /= $ptotal; # normalize to actual prob
    return $pgap;
}
...
sub GetColListGappy {
    ...
    my @colList=();
    my $col;
    for ($col=0; $col<$sto->GetLength(); $col++) {
	if ($sto->GetColGapWeight($col) > $maxGapFreq) {
	    push @colList,$col;
	}
    }
    return @colList;
...
sub RemoveColsGappy {
    my ($sto,$maxGapFreq)=@_;
    my @gappyList=$sto->GetColListGappy($maxGapFreq);
    $sto->RemoveColumnsInList(\@gappyList);
}
...
sub RemoveColumnsInList { # function could also be called "ProjectToNegationOfColList"
    my ($sto,$colListRef)=@_;
...
sub SortedSeqsAndNormalizedSSCons {
    ...
    for my $hit ($sto->ListHits()) {
	my $seq=$sto->GetSeq($hit);
	$seq =~ s/[^A-Za-z]/./g; # normalize all gaps to dots
	if (!$seenSeq{$seq}) {
	    push @seqList,$seq;
	    $seenSeq{$seq}=1;
	}
...
sub NormalizeSeqToRna {
    ...
    $seq=uc($seq);
    $seq =~ tr/T/U/;
    $seq =~ s/[^A-Za-z]/./g; # all gaps should be dots
    return $seq;
}

sub GetColListWhereGcIsNotGap {
    my ($sto,$gctag)=@_;
    my @l=();
    my $gcseq=$sto->{gc}{$gctag};
    if (!defined($gcseq)) {
	die "Stockholm did not have expected #=GC $gctag to extract core nucs";
    }
    for (my $i=0; $i<length($gcseq); $i++) {
	my $ch=substr($gcseq,$i,1);
	if (!IsGapCh($ch)) {
	    push @l,$i;
	}
    }
...
sub static_ProjectSeqByColList {
    my ($seq,$colListRef)=@_;
    my $projSeq="";
    for my $corePos (@$colListRef) {
	$projSeq .= substr($seq,$corePos,1);
    }
    return NormalizeSeqToRna($projSeq);
}

sub ProjectHitSeqByCORE {
    my ($sto,$hit)=@_;
    my @colList=$sto->GetColListWhereGcIsNotGap("CORE");
    my $seq=$sto->GetSeq($hit);
    my $projSeq=static_ProjectSeqByColList($seq,\@colList);
    return $projSeq;
}

sub RemoveHitsWhoseCORESeqDoesNotMatchGiven {
    my ($sto,$givenProjSeq)=@_;
    $givenProjSeq=NormalizeSeqToRna($givenProjSeq);
    my @colList=$sto->GetColListWhereGcIsNotGap("CORE");
    my @delHits=();
    for my $hit ($sto->ListHits()) {
	my $seq=$sto->GetSeq($hit);
	my $projSeq=static_ProjectSeqByColList($seq,\@colList);
	if ($projSeq ne $givenProjSeq) {
	    push @delHits,$hit;
	}
    }
    $sto->RemoveHits(@delHits);
}
sub RemoveHitsWhoseCORESeqDoesNotMatchCORESEQ {
    my ($sto)=@_;
    my $givenProjSeq=$sto->{gf}{CORESEQ};
    if (!defined($givenProjSeq)) {
	die "#=GF CORESEQ not found in .sto";
    }
    $sto->RemoveHitsWhoseCORESeqDoesNotMatchGiven($givenProjSeq);
}
...
sub GetFirstLastFromGCLine {
    my ($sto)=@_;
    my ($first,$last);
    my $s=$sto->{gc}{FirstLast};
    if (!defined($s)) {
	die;
    }
    if (!($s =~ /^[.]*<[.]*>[.]*$/)) {
	die "$s is not valid format";
    }
    $first=index($s,"<");
    $last=index($s,">");
    return ($first,$last);
...
sub static_GetFirstLastNonGap {
    my ($seq)=@_;
    my $first=0;
    my $last=length($seq)-1;
    while (1) { # assumes that PDB entry has >=1 non-gap character, which seems safe
	my $ch=substr($seq,$first,1);
	if ($ch eq "") {
	    die "seq=\"$seq\", first=\"$first\"";
	}
	if (!Stockholm::IsGapCh($ch)) {
	    last;
	}
	$first++;
    }
    while (1) {
	my $ch=substr($seq,$last,1);
	if ($ch eq "") {
	    die "seq=\"$seq\", last=\"$last\"";
	}
	if (!Stockholm::IsGapCh($ch)) {
	    last;
	}
